...
//设计算法将给定的表达式树（二叉树），转化为等价的中缀表达式，（通过括号反应操作符的计算次序），并输出
#define MAX 20
...
//		     a     b    c    -
//								d
void infix_expression(BiTree T,int deep)
{
	if (T == NULL)
		return;
	...
	else 
	{
		if (deep > 1)
			printf("(");
		infix_expression(T->left, deep + 1);
		printf("%c", T->data);
		infix_expression(T->right, deep + 1);
		if (deep > 1)
			printf(")");
	}
}
